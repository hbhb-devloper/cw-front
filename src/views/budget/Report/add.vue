    ...
    <el-col :span="24">
      <el-form :inline="true" label-width="160px">
        ...
        >
          <el-select
            placeholder="请选择科目类型"
            v-model="obj2.budgetId"
            class="input-select-length"
            filterable
            @change="handleType"
          >
            <el-option
        ...
        <el-form-item label="管理层审批附件">
          ...
          <div class="file-box">
            <div v-for="(item, index) in fileList" v-if="item.type == 1">
              <i class="el-icon-folder"></i
              ><span class="fileName">{{ item.name }}</span
              ><span
                @click="beforeRemove(item)"
        ...
        <el-form-item>
          ...
          <el-button
            class="btn"
            type="primary"
            v-if="obj2.state == 10 || obj2.state == 30"
            @click="handleSubmit"
            >修改保存
          </el-button>
          <el-button
            class="btn"
            type="primary"
            v-if="obj2.state == 31 || obj2.state == 40"
            @click="handleSubmit"
            >调整保存
          </el-button>
        </el-form-item>
...
export default {
  ...
  watch: {
    taxIncludeAmount(val) {
      if (!val) return;
      let countss =
        parseFloat(this.obj2.taxIncludeAmount) - parseFloat(this.obj2.cost);
  ...
  methods: {
    ...
    handleExceed(files, fileList) {
      this.$message.warning(
        `当前限制选择6个文件，本次选择了 ${files.length} 个文件，共选择了 ${
    ...
    imageUpload(param) {
      ...
      if (this.stutic != "add") {
        list.isApproved =
          this.obj2.state == 31 || this.obj2.state == 32 ? 1 : 0;
      }
    ...
    handleType() {
      ...
      this.obj2.engineeringNum = engineeringNum.substr(
        0,
        engineeringNum.indexOf("_")
      );
    ...
    handleSubmit() {
      ...
      if (
        (parseFloat(this.obj2.cost) + parseFloat(this.obj2.vatAmount)).toFixed(
          6
        ) != parseFloat(this.obj2.taxIncludeAmount)
      ) {
      ...
      if (parseFloat(this.obj2.amount) >= parseFloat(this.quota)) {
        ...
        if (fileLength == 0) {
          this.$message.warning(
            `项目预算总额超过${this.quota}万元，请上传管理层审批附件`
          );
          return;
        }
      }
      ...
      if (
        parseFloat(this.obj2.availableAmount) > parseFloat(this.obj2.amount) ||
        parseFloat(this.obj2.taxIncludeAmount) > parseFloat(this.obj2.amount)
      ) {
