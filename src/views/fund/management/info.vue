      ...
      <el-row :span="24" class="info-row">
        ...
        <el-col :span="12">
          ...
          <el-col :span="12">
            <el-input
              v-if="info.state == 20"
              v-model="form2.versions"
              type="number"
              :min="0"
              placeholder="请输入发票版本号"
            />
            <span v-else class="label-content">{{
              info.versions
            }}</span></el-col
          >
        </el-col>
          ...
          <el-table :data="fileTable" style="width: 100%">
            ...
            >
              <template style="color: #409eff" slot-scope="scope">
                <div @click="downfile(scope.row.filePath, scope.row.fileName)">
                  {{ scope.row.fileName }}
                </div>
              </template>
            </el-table-column>
