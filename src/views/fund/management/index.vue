  ...
  <div class="app-container">
    <el-form :model="queryParams"
      ...
      <el-form-item label="开票金额"
                    prop="invoiceAmount">
        <el-input v-model="queryParams.invoiceAmount"
                  placeholder="请输入开票金额"
                  clearable
                  size="small"
                  :min="0"
      ...
      <el-form-item label="客户经理"
                    prop="clientManager">
        <el-input v-model="queryParams.clientManager"
                  placeholder="请输入客户经理"
                  clearable
                  size="small"
                  style="width: 240px" />
      ...
      <el-form-item label="发票编号"
                    prop="invoiceNumber">
        <el-input v-model="queryParams.invoiceNumber"
                  placeholder="请输入发票编号"
                  clearable
                  size="small"
                  style="width: 240px" />
    ...
    <el-table v-loading="loading"
      ...
      <el-table-column prop="stateLabel"
                       ...
                       width="130">
        <template slot-scope="scope">
          <router-link :class="scope.row.isCancellation?'red':''"
                       style="color:#409EFF;font-size: 12px;"
                       :to="scope.row.isCancellation?'#':'/fund/management/info/'+scope.row.id">{{scope.row.stateLabel}}</router-link>
        </template>
      </el-table-column>
      ...
      <el-table-column width="130"
                       ...
                       label="发起流程">
        <template slot-scope="scope">
          <el-button v-if="scope.row.state!=30"
                     :class="scope.row.isCancellation?'red':''"
                     size="mini"
                     type="text"
                     :disabled="scope.row.state==20||scope.row.state==31"
      ...
      <el-table-column label="编辑到账"
        ...
        <template slot-scope="scope">
          <el-button :class="scope.row.isCancellation?'red':''"
                     size="mini"
                     type="text"
                     :disabled="scope.row.state!=31"
                     @click="handleEdits(scope.row)">编辑</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否作废"
        ...
        <template slot-scope="scope">
          <el-switch v-model="scope.row.isCancellation"
                     @change="handleTovoid(scope.row)">
          </el-switch>
        </template>
      </el-table-column>
      <el-table-column label="是否含文件"
                       width="130"
                       prop="isFile"
                       align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title"
      ...
      <el-form ref="form"
        ...
        <el-col :span="12">
          <el-form-item label="客户经理"
                        :required="true">
            <el-input v-model="form.clientManager"
                      :disabled="form.state==20"
                      placeholder="请输入客户经理" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="单位编号"
                        :required="true">
            <el-select v-model="form.unitNumber"
        ...
        <el-col :span="12"
                v-if="parseInt(form.business)==20">
          <el-form-item label="欠费时间"
                        :required="true">
            <el-date-picker v-model="form.arrearageMonth"
                            type="date"
                            format="yyyy-MM-dd"
        ...
        <el-col :span="12"
          ...
          <el-form-item label="欠费金额"
                        :required="true">
            <el-input v-model="form.arrearageMoney"
                      type="number"
                      min="0"
                      placeholder="请输入欠费金额" />
          </el-form-item>
        ...
        <el-col :span="12">
          <el-form-item label="计号费"
                        :required="true">
            <el-input v-model="form.billingNumber"
                      placeholder="请输入计号费" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="推送地址"
                        :required="true">
            <el-input v-model="form.pushAddress"
                      placeholder="请输入推送地址" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="发票版本号"
                        :required="true">
            <el-input v-model="form.versions"
                      disabled
                      placeholder="请输入发票版本号" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="发票编号"
        ...
        <el-col :span="12">
          <el-form-item label="到账时间"
                        :required="true">
            <el-date-picker v-model="form.accountTime"
                            type="date"
                            style="width: 100%"
                            disabled
                            format="yyyy-MM-dd"
        ...
        <el-col :span="12">
          <el-form-item label="到账金额"
                        :required="true">
            <el-input v-model="form.accountMoney"
                      disabled
                      type="number"
                      min="0"
                      placeholder="请输入到账金额" />
          ...
          <el-form-item label="附件"
            ...
            <el-upload class="upload-demo"
              ...
              <el-button size="small"
                         type="primary"
                         class="uploadImgBtn">添加</el-button>
            </el-upload>
            <div class="file-box">
              <div v-for="(item,index) in fileList"
                   :key="index"><i class="el-icon-folder"></i><span class="fileName">{{item.fileName}}</span><span @click="beforeRemove(item)"
    ...
    <el-dialog :title="title"
      ...
      <el-form ref="form"
        ...
        <el-form-item label="选择流程"
                      prop="flowName">
          <el-select v-model="flow.flowTypeId"
                     placeholder="请选择流程"
                     clearable
                     size="medium">
            <el-option v-for="dict in typeState"
                       :key="dict.id"
                       :label="dict.label"
                       :value="dict.id" />
          </el-select>
      ...
      <div slot="footer"
           class="dialog-footer">
        <el-button type="primary"
                   @click="submitForm2">确 定</el-button>
        <el-button @click="cancel">取 消</el-button>
      </div>
    </el-dialog>

    <el-dialog :title="'编辑到账'"
               :visible.sync="open2"
               width="480px">
      <el-form ref="form2"
               :model="form2"
        ...
        <el-form-item label="到账金额"
                      :required="true">
          <el-input v-model="form2.accountMoney"
                    type="number"
                    min="0"
                    placeholder="请输入到账金额" />
        </el-form-item>
        <el-form-item label="到账时间"
                      :required="true">
          <el-date-picker v-model="form2.accountTime"
                          type="date"
                          style="width: 100%"
                          format="yyyy-MM-dd"
                          value-format="yyyy-MM-dd"
      ...
      <el-button type="primary"
                 @click="submitForm3">确 定</el-button>
      <el-button @click="cancel">取 消</el-button>
    </el-dialog>
  </div>
</template>
