  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      >
        <template slot-scope="scope">
          <span :class="scope.row.isCancellation ? 'red' : ''">{{
            scope.row.invoiceCreateTime
              ? scope.row.invoiceCreateTime.substr(0, 19)
              : scope.row.invoiceCreateTime
          }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            :disabled="
              parseInt(scope.row.state) == 20 || parseInt(scope.row.state) == 31
            "
...
export default {
  ...
  methods: {
    ...
    handleBusiness() {
      if (
        parseInt(this.form.business) == 10 ||
        parseInt(this.form.business) == 20
      ) {
    ...
    submitForm: function () {
      ...
      if (this.form.invoiceAmount) {
        this.form.invoiceAmount = parseFloat(this.form.invoiceAmount).toFixed(
          2
        );
      }
      if (this.form.arrearageMoney) {
        this.form.arrearageMoney = parseFloat(this.form.arrearageMoney).toFixed(
          2
        );
      }
    ...
    //是否作废
    handleTovoid(row) {
      cancellation(row.id, row.isCancellation)
        .then((res) => {
          if (!res) {
            if (row.isCancellation) {
              this.msgSuccess("已作废");
