      ...
      <div>
        <el-col :span="24" :xs="24">
          <el-form ref="queryForm" :inline="true" label-width="100px">
            ...
            <el-form-item label="校验后六位：" prop="checkCode">
              <el-input
                placeholder="请输入关键词"
                v-model="obj2.checkCode"
                clearable
                type="number"
                size="small"
                maxlength="6"
        ...
        <el-dialog :visible.sync="centerDialogVisible">
          <el-form ref="queryForm" :inline="true" label-width="120px">
            <el-form-item label="发票种类代码：" :required="true">
              <el-select v-model="obj.invoiceType" placeholder="请选择关键词" style="width: 240px;" >
                <el-option label="增值税普通发票" value="04"></el-option>
                <el-option label="增值税电子发票" value="10"></el-option>
                <el-option label="增值税普通发票（卷式）" value="11"></el-option>
                <el-option label="增值税电子普通发票(通行费)" value="18"></el-option>
                ...
                <el-option label="航空电子客票行程单" value="23"></el-option>
            ...
            <el-form-item label="校验后六位：" :required="true">
              <el-input
                placeholder="请输入关键词"
                v-model="obj.checkCode"
                clearable
                size="small"
                style="width: 240px;"
              />
...
<script>
  import { getList, adddata, update, delData, delarr,getInfo } from '@/api/invoice/general_vat/general_vat.js'
  ...
  export default {
    ...
    methods: {
      ...
      //
      handleMax(){
        if(this.obj.taxFreeAmount>100000){
          this.$message.warning('金额不能超过100000');
        }
      },
      //信息提交
      handleSubmit() {
        ...
        this.obj.itime = dateTimes(this.obj.itime);
        if(this.obj.checkCode.length!=6){
          this.$message.warning('校验码必须输入六位');
          return;
        };
        if(!this.obj.invoiceCode||!this.obj.invoiceType||!this.obj.invoiceNumber||!this.obj.invoiceDate||!this.obj.checkCode){
      ...
      handleExport() {
        ...
        exportData(getToken(), this.delarrs, '/invoice/invoice_vat/export', '增值税普票');
      },
      handleBatchExport(){
        this.$confirm('是否按当前搜索条件导出', '提示', {
          ...
          BatchExport(getToken(), this.batch, '/invoice/invoice_vat/export/all', '增值税普票')
        }).catch(() => {
